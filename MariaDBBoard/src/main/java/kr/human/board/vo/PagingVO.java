...
public class PagingVO<T> {
	...
	public String getPageList() {
		...
		if(startPage-blockSize>=1) {
			pageList+="[<a href='?p="+(startPage-blockSize)+"&s="+pageSize+"&b="+blockSize+"'>◀◀</a>] ";
		}
		// 시작 페이지가 1보다 크면 이전이 있다.
		if(currentPage-1>=startPage) {
			pageList+="[<a href='?p="+(currentPage-1)+"&s="+pageSize+"&b="+blockSize+"'>◀</a>] ";
		}
		// 페이지 번호를 찍자
		...
		if(endPage<totalPage || currentPage+1<=endPage) {
			pageList+="[<a href='?p="+(currentPage+1)+"&s="+pageSize+"&b="+blockSize+"'>▶</a>] ";
		}
		
		if(endPage+blockSize<=totalPage) {
			pageList+="[<a href='?p="+(startPage+blockSize)+"&s="+pageSize+"&b="+blockSize+"'>▶▶</a>] ";
		}
	...
	public String toString() {
		return "PagingVO [list=" + list + ", totalCount=" + totalCount + ", currentPage=" + currentPage + ", pageSize="
				+ pageSize + ", blockSize=" + blockSize + ", totalPage=" + totalPage + ", startPage=" + startPage
